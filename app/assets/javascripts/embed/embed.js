(function($){	
	var _jQuizMeLayout = $("<div/>").addClass( "quiz-el cf").append( 
	...
	$.fn.jQuizMe = function( wordList, options, userLang, AJAX_increase_url ){
		...
		return this.each( function(){
			...
			changeQuizInfo = function(){
				...
				if( settings.intro ){
					...
					$( ".q-begin-btn", currQuiz ).bind("click",function(){
						$.post(AJAX_increase_url);
					});
				}			
			},
...
function embedQuiz(quiz_json, questions_json, answers_json, div_id, AJAX_increase_url){
	var quiz = JSON.parse(quiz_json);
	var questions = JSON.parse(questions_json);
	var answers = JSON.parse(answers_json);
	
	showQuiz(quiz, questions, answers, div_id, AJAX_increase_url);
}

function showQuiz(quiz, questions, answers, div_id, AJAX_increase_url){
	if (!quiz.description)
		{quiz.description = " ";}
    var options = { allRandom: 		false, //quiz.random,
				help: 			quiz.help_text,
				intro: 			quiz.description,
				review: 		quiz.allow_review,
				showFeedback:	quiz.show_answer,
    ...
    $( '#' + div_id ).children('.quizme_quizarea').jQuizMe( quiz, options, lang, AJAX_increase_url );
}
...
$(".quizme_share_btn").click(function(){$( this ).nextAll(".quizme_sharebuttons").slideToggle();});